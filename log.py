...
try:
    secrets = yaml.safe_load(
        open(os.path.join(os.path.dirname(__file__), "secrets.yml"))
    )
except OSError:
    secrets = None
...
async def discover_device():
    ...
    for d in all_devices:
        if (
            "manufacturer_data" in d.metadata.keys()
            and 65535 in d.metadata["manufacturer_data"].keys()
        ):
            # Specific device with CO2 sensor
            if (
                d.metadata["manufacturer_data"][65535][0] == 2
                and d.metadata["manufacturer_data"][65535][1] == 236
            ):
                devices.append(d)
    return devices
    ...
    for d in devices:
        ...
        readings.append(
            dict(
                ...
                press=(data[11] * 256 * 256 + data[10] * 256 + data[9])
                / 100,  # pressure in Pa
...
def print_sensor_reading(readings):
    for r in readings:
        ...
        print(
            f"{current_time}:  Device: {r['mac']}, CO2: {r['co2']} ppm, "
...
def write_to_csv(readings):
    for r in readings:
        filename = os.path.join(
            ...
            config["path"],
            f"data_{r['mac'].replace(':','_')}.csv",
        )
        # Check if file exists
        if not os.path.isfile(filename):
            with open(filename, "a") as f:
                f.write(
                    "Datetime,CO2 (ppm),T (°C),RH (%),P (Pa),Ambient Light (ADC),Battery (mV)\n"
                )
        with open(filename, "a") as f:
            f.write(
                f"{r['timestamp']},{r['co2']},{r['temp']},{r['humi']},{r['press']},{r['photo']},{r['batt']}\n"
            )
...
def send_alert(readings):
    for r in readings:
        if "E7" in r["mac"] and r["co2"] > config["alert_co2"]:
            ...
            if secrets:
                os.popen(
                    f"curl -X POST --data-urlencode 'payload={payload}' {secrets['mattermost_url']}"
                )
            else:
                print(payload)
        ...
        with InfluxDBClient(
            ...
            for r in readings:
                name = [
                    ...
                    if value.upper() == r["mac"].upper()
                ]
                point = (
                    Point(name[0] if len(name) > 0 else r["mac"])
                    .field("CO2 (ppm)", r["co2"])
                    .field("T (°C)", r["temp"])
                    .field("RH (%)", r["humi"])
                    .field("P (Pa)", r["press"])
...
async def run():
    ...
    while True:
        ...
        if len(devices) > 0:
            ...
            if config["sync"]:
                os.popen(
                    f"rsync -avhu {os.path.join(os.path.dirname(__file__), config['path'])} {config['sync_path']}"
                )
            # Alert to mattermost
            if config["alert"]:
                send_alert(readings)
